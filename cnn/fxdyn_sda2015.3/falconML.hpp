...
if(exec){
    ...
    ap_fixed<25, 2> wt_tmp[HWout_][HWin_];
    ap_fixed<18, 16> tmp[HWin_];
...
#pragma HLS UNROLL 
                        tmp[i] = (ap_fixed<18, 16>)in[i][r*Kstride+kr][c*Kstride+kc]; 
                        for(int o=0; o<HWout_; o++){
#pragma HLS UNROLL
                            wt_tmp[o][i] = (ap_fixed<25, 2>)weight[o][i][kr][kc]; 
