...
void memset_int(data_t *m, data_t val, int addr, int length) {
    for(int i=0; i<length; i++) {
        ...
        float tmp = (val+(data_t)(i%10)*0.0001);
        float tmp1 = ((int)rand()%2==0)?(tmp):(-1.0*tmp);//val + 0.05*(float)i; 
        m[i + addr] = (data_t)tmp1;//val + 0.05*(float)i; 
    }
}
template<typename data_t>
...
void prepare_weight(bw_t *DRAM_weight_reorder, data_t *DRAM_weight ) {
	...
	for(int i=0; i<WEIGHTSIZE/UNROLL; i++) {
        for(int j=0; j<UNROLL; j++) {
            ...
            ap_uint<DSIZE> *idata = (ap_uint<DSIZE>*)&data;
            tmp.range((j+1)*DSIZE-1, j*DSIZE) = *idata;
        }
        DRAM_weight_reorder[i] = tmp;
	}
}

void prepare_image(bw_t *m_fm, int addr_fm, float *m, int addr_m, int length) {
    int factor = UNROLL;
    bw_t data;
    for(int i=0; i<length/factor; i++) {
        for(int j=0; j<factor; j++) {
            data_t fdata = (data_t)m[addr_m + i*factor + j];
            ap_uint<DSIZE>* idata = (ap_uint<DSIZE>*) &fdata;
            data.range((j+1)*DSIZE-1, j*DSIZE) = *idata;
        }
        m_fm[i + addr_fm] = data;
    }
	data_t adata[UNROLL][224][224];
    factor = UNROLL;
    bw_t tmp;
	for(int i=0; i<3; i++) {
	    for(int j=0; j<224; j++) {
	        for(int k=0; k<224; k+=factor){
                for(int kk=0; kk<factor; kk++) {
	                ap_uint<DSIZE> tdata = m_fm[(i*224*224+j*224+k)/factor].range((kk+1)*DSIZE-1, kk*DSIZE);
                    data_t *fdata = (data_t*)&tdata;
	...
	for(int j=0; j<224; j++) {
	    for(int k=0; k<224; k++){
	        for(int i=0; i<UNROLL; i++) {
	            data_t fdata2 = adata[i][j][k];
                ap_uint<DSIZE> *idata = (ap_uint<DSIZE>*)&fdata2;
	            tmp.range((i+1)*DSIZE-1, i*DSIZE) = *idata;
	        }
            m_fm[(j*224+k)] = tmp;
	    }
	}
...
void reorder_output(data_t *m , wb_t *m_fm, int addr) {
   ...
   for(int ii=0; ii<num; ii+=UNROLL) {
       for(int r=0; r<row; r++) {
           for(int c=0; c<col; c++) {
               for(int i=0; i<UNROLL; i++) {
                   ap_uint<DSIZE> idata = m_fm[addr/UNROLL +(r*col+c) + (ii/UNROLL)*row*col].range((i+1)*DSIZE-1, i*DSIZE);
...
template<typename data_dst, typename data_src>
void mymemcpy(data_dst* DRAM_dst, data_src* DRAM_src, int length) {
    for(int i=0; i<length; i++){
        data_src tmp = DRAM_src[i];
        data_dst tmph = (data_dst)tmp;
        DRAM_dst[i] = tmph;
    }
}
...
int main(int argc, char** argv)
{
    ...
    DRAM_sw = (float*) malloc(swDramSize*sizeof(float));
    if (DRAM_sw==NULL) {
        printf("malloc failure\n");
        exit (1);
    }
    //memset_int<float>(DRAM_sw, (float)0.0010, 0, swDramSize);
    memset_int<float>(DRAM_sw, (float)1.0000, WEIGHTSIZE, 224*100);
    memset_int<float>(DRAM_sw, (float)1.0000, (WEIGHTSIZE+224*100), (224*124+224*324));
    memset_int<float>(DRAM_sw, (float)1.0000, (WEIGHTSIZE+(224*224+224*324)), 124*224);
    //initialize weight
    memset_int<float>(DRAM_sw, (float)0.0010, 0, Layer11);
    memset_int<float>(DRAM_sw, (float)0.0010, Layer11, Layer12-Layer11);
    memset_int<float>(DRAM_sw, (float)0.0040, Layer12, Layer21-Layer12);
    memset_int<float>(DRAM_sw, (float)0.0050, Layer21, Layer22-Layer21);
    memset_int<float>(DRAM_sw, (float)0.0012, Layer22, Layer31-Layer22);
    memset_int<float>(DRAM_sw, (float)0.0030, Layer31, Layer32-Layer31);
    memset_int<float>(DRAM_sw, (float)0.0020, Layer32, Layer33-Layer32);
    memset_int<float>(DRAM_sw, (float)0.0010, Layer33, Layer41-Layer33);
    memset_int<float>(DRAM_sw, (float)0.0010, Layer41, Layer42-Layer41);
    memset_int<float>(DRAM_sw, (float)0.0060, Layer42, Layer43-Layer42);
    memset_int<float>(DRAM_sw, (float)0.0010, Layer43, Layer51-Layer43);
    memset_int<float>(DRAM_sw, (float)0.0020, Layer51, Layer52-Layer51);
    memset_int<float>(DRAM_sw, (float)0.0040, Layer52, Layer53-Layer52);
    //initialize bias
    memset_int<float>(DRAM_sw, (float)0.0000, DATA*CONV11*KSIZE*KSIZE, CONV11);
    memset_int<float>(DRAM_sw, (float)0.0000, Layer12-CONV12, CONV12);
    memset_int<float>(DRAM_sw, (float)0.0000, Layer21-CONV21, CONV21);
    memset_int<float>(DRAM_sw, (float)0.0000, Layer22-CONV22, CONV22);
    memset_int<float>(DRAM_sw, (float)0.0000, Layer31-CONV31, CONV31);
    memset_int<float>(DRAM_sw, (float)0.0000, Layer32-CONV32, CONV32);
    memset_int<float>(DRAM_sw, (float)0.0000, Layer33-CONV33, CONV33);
    memset_int<float>(DRAM_sw, (float)0.0000, Layer41-CONV41, CONV41);
    memset_int<float>(DRAM_sw, (float)0.0000, Layer42-CONV42, CONV42);
    memset_int<float>(DRAM_sw, (float)1.0000, Layer43-CONV43, CONV43);
    memset_int<float>(DRAM_sw, (float)1.0000, Layer51-CONV51, CONV51);
    memset_int<float>(DRAM_sw, (float)1.0000, Layer52-CONV52, CONV52);
    memset_int<float>(DRAM_sw, (float)1.0000, Layer53-CONV53, CONV53);
    ...
    data_t *DRAM_weight = (data_t*) malloc(DramSize_weight*DSIZE/8);
    bw_t *DRAM_weight_reorder = (bw_t*) malloc(DramSize_weight*DSIZE/8);
    bw_t *DRAM_featmap = (bw_t*) malloc(DramSize_featmap*DSIZE/8);
    bw_t *DRAM_hw = (bw_t*) malloc(DramSize_hw*DSIZE/8);
    printf("1. init DRAM_hw finish\n");

    mymemcpy<data_t, float>(DRAM_weight, DRAM_sw, WEIGHTSIZE);
    ...
    bw_t *DRAM_ly = (bw_t*) malloc(LayerDef*DSIZE/8);
    for(int a=0; a<14; a++) {
        bw_t tmp=0;
        for(int b=0; b<16; b++){
           if(b<16) { 
                tmp.range(2*DSIZE*(b+1)-1, 2*DSIZE*b) = (ap_uint<2*DSIZE>)ly[a*16+b]; 
           }
        } 
        DRAM_ly[a] = tmp; 
    }
    ...
    bwmemcpy(DRAM_hw, 0, DRAM_ly, 0, 14); 
...
//layer[1]
    ...
    data_t *DRAM_result = (data_t*)malloc(FM11*DSIZE/8);
...
//layer[1~2]
    ...
    data_t tmp_hw=0;
    data_t sum=0;
    data_t *DRAM_result = (data_t*)malloc(PL12*DSIZE/8);
...
//layer[1~2] without pool
    ...
    data_t tmp_hw=0;
    data_t sum=0;
    data_t *DRAM_result = (data_t*)malloc(FM12*DSIZE/8);
...
//layer[1~3]
    ...
    data_t tmp_hw=0;
    data_t sum=0;
    data_t *DRAM_result = (data_t*)malloc(PL33*DSIZE/8);
...
//layer[1~5]
    ...
    float tmp_hw=0;
    data_t sum=0;
    data_t *DRAM_result = (data_t*)malloc(PL53*DSIZE/8);
    ...
    for(int i=0; i<PL53; i++) {
        tmp_sw = DRAM_sw[WEIGHTSIZE+ INFM+ i];
        data_t tmp_half = DRAM_result[i];
        tmp_hw = (float)tmp_half;
        if (cmpratio(tmp_sw, tmp_hw)>5E-2) {
            printf("sw:%f,\t hw:%f,\t ratio:%f\n", (float)tmp_sw, (float)tmp_hw, (float)cmpratio(tmp_sw, tmp_hw));
            cnt+=1;
        }
        final << "\t\tsw: " <<  (float)tmp_sw << "\thw: " << (float)tmp_hw << "\tratio:" << (float)cmpratio(tmp_sw, tmp_hw)<< endl;
