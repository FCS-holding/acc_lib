    ...
    if(i_symSize == 30){
        if(hpLen < 2){
            ...
            #pragma HLS LOOP_TRIPCOUNT max = 2 min = 1
                ...
                //printf("smLen %d \n", smLen);
                uint32_t node;
                node = heap[++hpLen].value = (smLen < 2? ++smLen:0);
                heap[node].frequency = 1;
...
void zlibTreegenInStremOutStream(hls::stream<uint32_t>& lit_freq,
                              hls::stream<uint32_t>& dist_freq,
...
#pragma HLS PIPELINE II = 1
        //inFreq[i] = (Frequency)lit_freq[i];
        inFreq[i] = (Frequency)lit_freq.read();
    }
    offset = offsets[1]; // copy distances
    for (int i = 0; i < c_dstCodeCount; ++i) {
#pragma HLS PIPELINE II = 1
        //inFreq[i + offset] = (Frequency)dist_freq[i];
        inFreq[i + offset] = (Frequency)dist_freq.read();
    }
    offset = offsets[2];
    for (int i = 0; i < c_blnCodeCount; ++i) {
        inFreq[i + offset] = 0; // just initialize
