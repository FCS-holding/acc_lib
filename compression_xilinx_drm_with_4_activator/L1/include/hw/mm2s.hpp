...
#pragma HLS PIPELINE II = 1
            lcl_ltree_codes[blk][ci] = dyn_ltree_codes[blk].read(); // Extract the data
            //lcl_ltree_codes[blk][ci] = dyn_ltree_codes[blk * c_lTreeSize + ci];
        }
    cpy_ltree_blen:
        for (uint32_t ci = 0; ci < c_lTreeSize; ++ci) {
#pragma HLS PIPELINE II = 1
            lcl_ltree_blen[blk][ci] = dyn_ltree_blen[blk].read(); // Extract the data                                           
            //lcl_ltree_blen[blk][ci] = dyn_ltree_blen[blk * c_lTreeSize + ci];
        }
    cpy_dtree_codes:
        for (uint32_t ci = 0; ci < c_dTreeSize; ++ci) {
#pragma HLS PIPELINE II = 1
            lcl_dtree_codes[blk][ci] = dyn_dtree_codes[blk].read(); // Extract the data                                                                 
            //lcl_dtree_codes[blk][ci] = dyn_dtree_codes[blk * c_dTreeSize + ci];
        }
    cpy_dtree_blen:
        for (uint32_t ci = 0; ci < c_dTreeSize; ++ci) {
#pragma HLS PIPELINE II = 1
            lcl_dtree_blen[blk][ci] = dyn_dtree_blen[blk].read(); // Extract the data                                                      
            //lcl_dtree_blen[blk][ci] = dyn_dtree_blen[blk * c_dTreeSize + ci];
        }
    cpy_bltree_codes:
        for (uint32_t ci = 0; ci < c_bLTreeSize; ++ci) {
#pragma HLS PIPELINE II = 1
            lcl_bltree_codes[blk][ci] = dyn_bltree_codes[blk].read(); // Extract the data                                                                  
            //lcl_bltree_codes[blk][ci] = dyn_bltree_codes[blk * c_bLTreeSize + ci];
        }
    cpy_bltree_blen:
        for (uint32_t ci = 0; ci < c_bLTreeSize; ++ci) {
#pragma HLS PIPELINE II = 1
            lcl_bltree_blen[blk][ci] = dyn_bltree_blen[blk].read(); // Extract the data                                                                  
...
cpy_maxcodes:
    for (uint32_t blk = 0; blk < PARALLEL_BLOCK; blk++) {
        for (uint32_t ci = 0; ci < c_maxCodeSize; ++ci) {
#pragma HLS PIPELINE II = 1
            lcl_maxcode[ci] = dyn_maxcodes[blk].read(); // Extract the data                                                              
            //lcl_maxcode[ci] = dyn_maxcodes[blk*c_maxCodeSize + ci];
        }
    }
