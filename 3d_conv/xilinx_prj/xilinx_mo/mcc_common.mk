...
CXX_ACC_HW_FLAGS=$(CXX_ACC_FLAGS) -D MCC_ACC_H_FILE=\"$(KERNEL_NAME).h\" \
                  -l$(KERNEL_NAME) 
...
ifdef AUTODSE
	CMP_OPT += --attribute auto_dse=on --attribute dse_hls_time=30 --attribute dse_algo=heuristic_70 --attribute dse_time=480
	LNK_OPT += --attribute auto_dse=on --attribute dse_hls_time=30 --attribute dse_algo=heuristic_70 --attribute dse_time=480
endif
...
default:
	...
	@echo "opencv"
	@echo "$$ make <target>"
	@echo "THE FOLLOWING TARGETS ALL USE VENDOR'S SELF-CONTAINED MAKEFILES:"
	@echo
	@echo "    vendor_host - build host program"
	@echo "   vendor_swsim - build sw simulation"
	@echo "   vendor_hwsim - build hw simulation"
	@echo "     vendor_run - run the sw simulation"
	@echo "vendor_estimate - run the hw simulation and generate estimate report"
	@echo "  vendor_bitgen - generate bitstream"
	@echo "   vendor_clean - clean generated files"
	@echo
...
$(BITGEN_OUTPUT) : $(ACCGEN_OUTPUT).mco
	...
	mv $(BITGEN_OUTPUT) $(BITGEN_OUTPUT_TIMESTAMPED)
	ln -s $(BITGEN_OUTPUT_TIMESTAMPED) $(BITGEN_OUTPUT)

mcc_estimate : $(EST_OUTPUT)

$(EST_OUTPUT) : $(ACCGEN_OUTPUT).mco
	merlincc $^ --report=estimate $(LNK_OPT) --platform=$(PLATFORM) -d11

mcc_runsim: mcc_simgen $(ACC_EXE)
ifdef __use_src__
	make vendor_run
else
	$(SIM_ENV)=$(SIM_ENV_VAL) ./$(ACC_EXE) $(EXE_ARGS) $(SIMGEN_OUTPUT).$(BIN_EXT) > log
endif

mcc_simgen : $(SIMGEN_OUTPUT).$(BIN_EXT) copy
ifdef __use_src__
#	install -D $< $(RELPATH_VENDOR_BINARY)
	install -D $< src/xclbin/${VENDOR_CONFIG_NAME}.${TARGET}.${DEVICE}.xclbin
#	-rm -rf src/xclbin/;
#	-mkdir src/xclbin;
#	-cp $< src/xclbin/${VENDOR_CONFIG_NAME}.${TARGET}.${DEVICE}.xclbin
endif


$(SIMGEN_OUTPUT).$(BIN_EXT) : $(ACCGEN_OUTPUT).mco
	merlincc $^ -march=$(SIM_ENV_VAL) -D MCC_SIM -o $(SIMGEN_OUTPUT) $(LNK_OPT) --platform=$(PLATFORM)
...
$(ACCGEN_OUTPUT).mco : $(KERNEL_SRC_FILES)
	merlincc -c $^ -D $(VENDOR) -o $(ACCGEN_OUTPUT) $(CMP_OPT) $(KERNEL_INC_DIR) --platform=$(PLATFORM) -d11
...
clean:
	rm -rf merlin.log *.o $(SRC_DIR)/data_int*.txt $(EXE)
...
copy:
	if [ ! -d src ]; then cp -r ../src src; fi
vendor_host: copy
	make -C src exe $(OVERRIDE)
vendor_swsim: copy
	make -C src all TARGETS=sw_emu $(OVERRIDE)
	cd src; $(XILINX_SDX)/bin/emconfigutil --platform $(DEVICE) --nd 1; XCL_EMULATION_MODE=sw_emu ./$(ACC_EXE) $(EXE_ARGS)
vendor_hwsim: copy
	make -C src all TARGETS=hw_emu $(OVERRIDE)
	cd src; $(XILINX_SDX)/bin/emconfigutil --platform $(DEVICE) --nd 1; XCL_EMULATION_MODE=hw_emu ./$(ACC_EXE) $(EXE_ARGS)
vendor_run: copy
	cd src; $(XILINX_SDX)/bin/emconfigutil --platform $(DEVICE) --nd 1; XCL_EMULATION_MODE=$(SIM_ENV_VAL) ./$(ACC_EXE) $(EXE_ARGS) 
vendor_estimate: copy
	export LDCLFLAGS="-R estimate"; make -C src all TARGETS=hw_emu $(OVERRIDE)
vendor_bitgen: copy
	make -C src all TARGETS=hw $(OVERRIDE)
vendor_clean:
	rm -rf src
#	make -C src clean
