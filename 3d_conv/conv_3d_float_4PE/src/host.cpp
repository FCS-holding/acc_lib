...
int main(int argc, char * argv[]){
    ...
    for(int i = 0; i < IN_SIZE_3; i++){
        data_in_merlin_3[i] = 0;
    }
    #endif
    ...
    for(int i=0; i<IMAGE_SIZE; i++) {
        for(int j=0; j<IN_LENGTH_3; j++) {
            for(int k=0; k<IMAGE_SIZE; k++) {
                data_in_merlin_3[i*IMAGE_SIZE*IN_LENGTH_3 + j*IMAGE_SIZE + k] \
                    = data_in[i*DATA_IN_LENGTH/image_depth + (j+START_3)*IMAGE_SIZE + k];
                //printf("data_in_merlin_3 :%4d, %f\n",i*IMAGE_SIZE*IN_LENGTH_3 + j*IMAGE_SIZE + k, data_in_merlin_3[i*IMAGE_SIZE*IN_LENGTH_3 + j*IMAGE_SIZE + k]);
            }
        }
    }
    #endif
    ...
    for(int i=0; i<OUT_IMAGE_SIZE; i++) {
        for(int j=0; j<OUT_IMAGE_SIZE; j++) {
            for(int k=0; k<OUT_IMAGE_SIZE; k++) {
                ...
                } else if(j < OUT_DEPTH_0+OUT_DEPTH_1 + OUT_DEPTH_2+OUT_DEPTH_3) {
                    data_out_merlin[i*OUT_IMAGE_SIZE*OUT_IMAGE_SIZE + j*OUT_IMAGE_SIZE + k] \
                    = data_out_merlin_3[i*OUT_IMAGE_SIZE*STEP + (j-OUT_DEPTH_0-OUT_DEPTH_1-OUT_DEPTH_2)*OUT_IMAGE_SIZE + k];
                #endif
                }
            }
        }
