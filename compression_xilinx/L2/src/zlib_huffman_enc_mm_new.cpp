...
#pragma HLS dataflow
    ...
    xf::compression::details::bitPacking(bitVals, bitLen, outStream, outStreamEos, compressedSize);
...
#pragma HLS dataflow
    ...
    #pragma HLS unroll
        huffmanCore(in[i], outStream[i], outStreamEos[i], input_size[i], inStreamTree[i], inStreamSize[i], compressedSize[i]);
    }
...
void xilHuffmanKernel(//uint32_t wr_block_idx,
    ...
    //uint32_t max_block_size = block_size_in_kb * 1024 * 33/32;
    //uint32_t max_block_size = 32 * 1024 * 33/32;
    uint32_t max_block_size = 33792;
