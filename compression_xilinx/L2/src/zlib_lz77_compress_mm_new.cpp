...
#pragma HLS dataflow
    ...
    xf::compression::details::mm2multStreamSize<8, PARALLEL_BLOCK, GMEM_DWIDTH, GMEM_BURST_SIZE>(in, input_idx,inStream, input_size);
    for (uint8_t i = 0; i < PARALLEL_BLOCK; i++) {
#pragma HLS UNROLL
        // lz77Core is instantiated based on the PARALLEL BLOCK
        lz77Core(inStream[i], lz77_out[i], lz77_out_eos[i],  dyn_ltree_freq[i], dyn_dtree_freq[i],
